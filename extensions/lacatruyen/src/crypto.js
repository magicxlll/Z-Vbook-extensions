/**
 * Pure ES5 AES-256-CBC with OpenSSL EvpKDF (MD5)
 * Self-contained, lightweight (~300 lines), zero external dependencies.
 * 100% compatible with Duktape / QuickJS / vBook on Android & iOS.
 */
...
function cipherBlock(block, roundKeys) {
    ...
    for (var round = 1; round <= 13; round++) {
        ...
        for (var c = 0; c < 4; c++) {
            ...
            state[i0] = gmul(2, a0) ^ gmul(3, a1) ^ a2 ^ a3;
            state[i1] = a0 ^ gmul(2, a1) ^ gmul(3, a2) ^ a3;
            state[i2] = a0 ^ a1 ^ gmul(2, a2) ^ gmul(3, a3);
            state[i3] = gmul(3, a0) ^ a1 ^ a2 ^ gmul(2, a3);
        }
...
function invCipherBlock(block, roundKeys) {
    var state = block.slice(0);
    ...
    for (var round = 13; round >= 1; round--) {
        ...
        for (var c = 0; c < 4; c++) {
            var i0 = c * 4, i1 = i0 + 1, i2 = i0 + 2, i3 = i0 + 3;
            var a0 = state[i0], a1 = state[i1], a2 = state[i2], a3 = state[i3];
            state[i0] = gmul(0x0e, a0) ^ gmul(0x0b, a1) ^ gmul(0x0d, a2) ^ gmul(0x09, a3);
            state[i1] = gmul(0x09, a0) ^ gmul(0x0e, a1) ^ gmul(0x0b, a2) ^ gmul(0x0d, a3);
            state[i2] = gmul(0x0d, a0) ^ gmul(0x09, a1) ^ gmul(0x0e, a2) ^ gmul(0x0b, a3);
            state[i3] = gmul(0x0b, a0) ^ gmul(0x0d, a1) ^ gmul(0x09, a2) ^ gmul(0x0e, a3);
        }
    }
...
function encryptAES(plainText, password) {
    ...
    try {
        ...
        for (var i = 0; i < padded.length; i += 16) {
            ...
            var enc = cipherBlock(block, roundKeys);
...
function decryptAES(cipherBase64, password) {
    ...
    try {
        ...
        for (var i = 0; i < cipherBytes.length; i += 16) {
            ...
            var dec = invCipherBlock(block, roundKeys);
            for (var b = 0; b < 16; b++) {
